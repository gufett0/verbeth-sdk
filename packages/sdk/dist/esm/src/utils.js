...
import { Contract, verifyMessage, hashMessage } from "ethers";
 ...
 */
export async function verifyEIP1271Signature(contractAddress, messageHash, signature, provider) {
    try {
        const accountContract = new Contract(contractAddress, ["function isValidSignature(bytes32, bytes) external view returns (bytes4)"], provider);
...
export async function isSmartContract(address, provider) {
    try {
        ...
        try {
            const contract = new Contract(address, ["function isValidSignature(bytes32, bytes) external view returns (bytes4)"], provider);
 ...
 */
export function verifyDerivationProof(derivationProof, expectedSenderAddress, expectedUnifiedKeys) {
    try {
        // 1. Verify the derivation signature was created by the expected address
        const recoveredAddress = verifyMessage(derivationProof.message, derivationProof.signature);
        ...
        const identityMatches = Buffer.from(boxKeyPair.publicKey).equals(Buffer.from(expectedUnifiedKeys.identityPubKey));
        const signingMatches = Buffer.from(signKeyPair.publicKey).equals(Buffer.from(expectedUnifiedKeys.signingPubKey));
        if (!identityMatches) {
 ...
 */
export function verifyEOADerivationProof(derivationProof, expectedSenderAddress, expectedUnifiedKeys) {
 ...
 */
export async function verifySmartAccountDerivationProof(derivationProof, smartAccountAddress, expectedUnifiedKeys, provider) {
    try {
        // 1. Verify using EIP-1271
        const messageHash = hashMessage(derivationProof.message);
        const isValidSignature = await verifyEIP1271Signature(smartAccountAddress, messageHash, derivationProof.signature, provider);
        if (!isValidSignature) {
