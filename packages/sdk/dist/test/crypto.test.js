...
import { encryptMessage, decryptMessage, encryptStructuredPayload, decryptStructuredPayload, decryptHandshakeResponse, decryptAndExtractHandshakeKeys } from '../src/crypto.js';
import { encodeHandshakePayload, decodeHandshakePayload, encodeHandshakeResponseContent, decodeHandshakeResponseContent, encodeUnifiedPubKeys, extractKeysFromHandshakePayload, extractKeysFromHandshakeResponse, parseHandshakeKeys } from '../src/payload.js';
describe('Encryption/Decryption', () => {
    describe('Message Encryption', () => {
        it('should encrypt and decrypt a message successfully', () => {
            ...
            const encrypted = encryptMessage(message, recipientKey.publicKey, senderBoxKey.secretKey, senderBoxKey.publicKey, senderSignKey.secretKey, senderSignKey.publicKey);
        ...
        it('should fail to decrypt if payload is tampered', () => {
            ...
            let encrypted = encryptMessage(message, recipientKey.publicKey, senderBoxKey.secretKey, senderBoxKey.publicKey, senderSignKey.secretKey, senderSignKey.publicKey);
        ...
        it('should work with the structured message format', () => {
            ...
            const encrypted = encryptStructuredPayload(messagePayload, recipientKey.publicKey, senderBoxKey.secretKey, senderBoxKey.publicKey, senderSignKey.secretKey, senderSignKey.publicKey);
            const decrypted = decryptStructuredPayload(encrypted, recipientKey.secretKey, (obj) => obj, senderSignKey.publicKey);
        ...
        it('should encrypt without signing keys (optional parameters)', () => {
            ...
            const encrypted = encryptMessage(message, recipientKey.publicKey, senderBoxKey.secretKey, senderBoxKey.publicKey);
        ...
        it('should encrypt structured payload without signing keys', () => {
            ...
            const encrypted = encryptStructuredPayload(messagePayload, recipientKey.publicKey, senderBoxKey.secretKey, senderBoxKey.publicKey);
            const decrypted = decryptStructuredPayload(encrypted, recipientKey.secretKey, (obj) => obj);
            expect(decrypted).toEqual(messagePayload);
        });
    });
    describe('Handshake Response Encryption', () => {
        it('should encrypt and decrypt handshake response content', () => {
            ...
            const encrypted = encryptStructuredPayload(responseContent, initiatorEphemeralKey.publicKey, responderEphemeralKey.secretKey, responderEphemeralKey.publicKey);
            const decrypted = decryptHandshakeResponse(encrypted, initiatorEphemeralKey.secretKey);
    ...
    describe('decryptAndExtractHandshakeKeys', () => {
        it('should decrypt and extract all keys from handshake response', () => {
            ...
            const encrypted = encryptStructuredPayload(responseContent, initiatorEphemeralKey.publicKey, responderEphemeralKey.secretKey, responderEphemeralKey.publicKey);
            const result = decryptAndExtractHandshakeKeys(encrypted, initiatorEphemeralKey.secretKey);
        ...
        it('should return null for invalid encrypted data', () => {
            ...
            const result = decryptAndExtractHandshakeKeys(invalidEncrypted, initiatorEphemeralKey.secretKey);
        ...
        it('should return null with wrong decryption key', () => {
            ...
            const encrypted = encryptStructuredPayload(responseContent, initiatorEphemeralKey.publicKey, responderEphemeralKey.secretKey, responderEphemeralKey.publicKey);
            const result = decryptAndExtractHandshakeKeys(encrypted, wrongKey.secretKey);
            expect(result).toBeNull();
        });
    });
    ...
    describe('Error Handling', () => {
        it('should throw error for malformed JSON in decryptStructuredPayload', () => {
            ...
            expect(() => {
                decryptStructuredPayload(malformedJson, recipientKey.secretKey, (obj) => obj);
            }).toThrow();
        });
        it('should throw error for missing fields in payload', () => {
            ...
            expect(() => {
                decryptStructuredPayload(incompletePayload, recipientKey.secretKey, (obj) => obj);
            }).toThrow();
        });
        it('should throw error for invalid base64 in payload fields', () => {
            ...
            expect(() => {
                decryptStructuredPayload(invalidPayload, recipientKey.secretKey, (obj) => obj);
            }).toThrow();
        });
        it('should return null when converter function throws error', () => {
            ...
            const throwingConverter = (obj) => {
                throw new Error('Converter error');
            };
            expect(() => {
                decryptStructuredPayload(encrypted, recipientKey.secretKey, throwingConverter);
        ...
        it('should throw error for decryptHandshakeResponse with missing derivationProof', () => {
            ...
            const invalidContent = {
                unifiedPubKeys: encodeUnifiedPubKeys(new Uint8Array(32).fill(40), new Uint8Array(32).fill(41)),
                ephemeralPubKey: new Uint8Array(32).fill(42),
                note: 'missing proof'
                // No derivationProof
            };
            const encrypted = encryptStructuredPayload(invalidContent, initiatorEphemeralKey.publicKey, responderEphemeralKey.secretKey, responderEphemeralKey.publicKey);
            expect(() => {
    ...
    describe('Log Message Structure', () => {
        it('should decode and decrypt a log message', () => {
            ...
            const ciphertext = encryptMessage(message, recipientKey.publicKey, senderBoxKey.secretKey, senderBoxKey.publicKey, senderSignKey.secretKey, senderSignKey.publicKey);
            const mockLog = {
