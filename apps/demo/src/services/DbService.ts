...
export class DbService {
  ...
  async saveIdentity(identity: StoredIdentity) {
    ...
    console.log(`🔑 Saving identity for ${normalizedAddress.slice(0, 8)}... (original: ${identity.address.slice(0, 8)})`);
    ...
    try {
      ...
      console.log(`✅ Identity saved successfully for ${normalizedAddress.slice(0, 8)}...`);
      
      // Verify it was saved
      const verification = await this.db.identity.get(normalizedAddress);
      if (verification) {
        console.log(`✅ Identity verified in DB for ${normalizedAddress.slice(0, 8)}...`);
      } else {
        console.error(`❌ Identity NOT found after save for ${normalizedAddress.slice(0, 8)}...`);
      }
      
      return result;
    } catch (error) {
      console.error(`❌ Failed to save identity for ${normalizedAddress.slice(0, 8)}...:`, error);
      throw error;
    }
  }
  ...
  async getIdentity(address: string) {
    ...
    console.log(`🔍 Looking for identity: ${normalizedAddress.slice(0, 8)}... (original: ${address.slice(0, 8)})`);
    
    try {
      const result = await this.db.identity.get(normalizedAddress);
      if (result) {
        console.log(`✅ Found identity for ${normalizedAddress.slice(0, 8)}... (derived: ${new Date(result.derivedAt).toLocaleString()})`);
      } else {
        console.log(`❌ No identity found for ${normalizedAddress.slice(0, 8)}...`);
      ...
      console.error(`❌ Error getting identity for ${normalizedAddress.slice(0, 8)}...:`, error);
      return null;
    }
  }
  ...
  saveContact(contact: Contact) {
    ...
    console.log(`👤 Saving contact ${normalizedContact.address.slice(0, 8)}... for owner ${normalizedContact.ownerAddress.slice(0, 8)}...`);
  ...
  getContact(address: string, ownerAddress: string) {
    ...
    return this.db.contacts.where('[address+ownerAddress]').equals([normalizedAddress, normalizedOwner]).first();
  }
  ...
  async saveMessage(message: Message): Promise<boolean> {
    ...
    const normalizedMessage = {
      ...
      recipient: message.recipient ? this.normalizeAddress(message.recipient) : undefined,
      ownerAddress: this.normalizeAddress(message.ownerAddress)
    };

    console.log(`💬 Saving new message from ${normalizedMessage.sender.slice(0, 8)}... for owner ${normalizedMessage.ownerAddress.slice(0, 8)}...`);
  ...
  getMessagesByContact(contact: string, ownerAddress: string, limit = 50) {
    ...
    return this.db.messages
      .where('ownerAddress')
      .equals(normalizedOwner)
      .filter(m => m.sender === normalizedContact || m.recipient === normalizedContact)
      .reverse()
      .limit(limit)
      .toArray();
  }
  ...
  async getAllMessages(ownerAddress: string, limit = 100) {
    ...
    const sorted = messages.sort((a, b) => b.blockTimestamp - a.blockTimestamp).slice(0, limit);
    console.log(`✅ Found ${sorted.length} messages for owner ${normalizedOwner.slice(0, 8)}...`);
    return sorted;
  }
  ...
  savePendingHandshake(h: PendingHandshake) {
    ...
    console.log(`🤝 Saving pending handshake from ${normalizedHandshake.sender.slice(0, 8)}... for owner ${normalizedHandshake.ownerAddress.slice(0, 8)}...`);
  ...
  async getAllPendingHandshakes(ownerAddress: string) {
    const normalizedOwner = this.normalizeAddress(ownerAddress);
    console.log(`🤝 Loading pending handshakes for owner ${normalizedOwner.slice(0, 8)}...`);
    ...
    console.log(`✅ Found ${sorted.length} pending handshakes for owner ${normalizedOwner.slice(0, 8)}...`);
    return sorted;
  }
  ...
  async clearUserData(addr: string) {
    ...
    await this.db.transaction(
      ...
      async () => {
        ...
        await this.db.contacts.where('ownerAddress').equals(normalizedAddr).delete();
        await this.db.messages.where('ownerAddress').equals(normalizedAddr).delete();
        await this.db.pendingHandshakes.where('ownerAddress').equals(normalizedAddr).delete();
  ...
  async exportData() {
    ...
    console.log(`✅ Exported ${payload.identity.length} identities, ${payload.contacts.length} contacts, ${payload.messages.length} messages`);
  ...
  async switchAccount(newAddress: string) {
    ...
    console.log(`🔄 Switching to account: ${normalizedAddress.slice(0, 8)}... (original: ${newAddress.slice(0, 8)})`);
    ...
    allIdentities.forEach(id => {
      console.log(`  - ${id.address} (derived: ${new Date(id.derivedAt).toLocaleString()})`);
    });
    ...
    console.log(`📊 Data for ${normalizedAddress.slice(0, 8)}...: ${contacts.length} contacts, ${messages.length} messages, ${handshakes.length} pending handshakes`);
  }
