...
import { ExecutorFactory, EOAExecutor, UserOpExecutor, DirectEntryPointExecutor, sendEncryptedMessage, initiateHandshake, respondToHandshake } from "../src/index.js";
...
describe("ExecutorFactory", () => {
    ...
    it("creates UserOp executor with correct parameters", () => {
        const executor = ExecutorFactory.createUserOp(TEST_SMART_ACCOUNT_ADDRESS, TEST_ENTRYPOINT_ADDRESS, TEST_LOGCHAIN_ADDRESS, mockBundler, mockSmartAccount);
        expect(executor).toBeInstanceOf(UserOpExecutor);
    });
    it("creates DirectEntryPoint executor for testing", () => {
        const executor = ExecutorFactory.createDirectEntryPoint(TEST_SMART_ACCOUNT_ADDRESS, mockEntryPoint, TEST_LOGCHAIN_ADDRESS, mockSmartAccount, mockSigner);
